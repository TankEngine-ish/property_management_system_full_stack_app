            ...
            steps {
                withCredentials([usernamePassword(credentialsId: 'nexus-docker-credentials', usernameVariable: 'NEXUS_USERNAME', passwordVariable: 'NEXUS_PASSWORD')]) {
                    sh '''
                        ...
                        docker tag nextapp:1.0.1 $DOCKER_HOSTED/nextapp:1.0.1
                        docker push $DOCKER_HOSTED/nextapp:1.0.1

                        docker tag goapp:1.0.1 $DOCKER_HOSTED/goapp:1.0.1
                        docker push $DOCKER_HOSTED/goapp:1.0.1
                    '''
                }
            }
        }
